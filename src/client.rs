...
async fn handle_message_success(
    ...
    println!("Received from authenticated user {} ({}): {:?}", user_id, client_addr, msg);
        ...
        match stream.read(&mut buffer).await {
            ...
            Ok(n) => {
                if authenticated_user_id.is_none() {
                    ...
                    match serde_json::from_str::<ClientMessage>(&received) {
                        Ok(msg) => handle_unauthenticated_message(stream, Arc::clone(&conn), client_addr, msg, &mut authenticated_user_id).await?,
                        Err(e) => handle_unauthenticated_parse_error(stream, client_addr, e).await?,
                    }
                } else {
                    // User is authenticated, allow normal communication
                    handle_client_message(stream, client_addr, authenticated_user_id.unwrap(), &buffer[..n]).await?;
                }
            }
            Err(e) => {
...
pub async fn handle_client(
    ...
    let auth_prompt = ClientMessage {
        ...
        data: "Please authenticate by sending 'login' or 'register' command with your credentials".to_string(),
    };
